...
test("irctc train ticket", async ({ page }) => {
  test.setTimeout(120000);
    await page.goto("https://www.irctc.co.in/nget/train-search", { waitUntil: "load", timeout: 120000 });
    await page.waitForSelector("//input[@aria-label='Enter From station. Input is Mandatory.']", { timeout: 120000 });
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!(await button.isVisible())) {
        console.log(`refresh button ${i} not visible, skipping`);
        continue;
      }
      ...
      try {
        ...
        console.log(`button ${i} click error: ${error.message}`);
      }
    }

    const price = await page.locator("//span[@class='ng-star-inserted']/strong").allTextContents();
   console.log(price);
   
    
    const sliderHandle = page.locator("(//span[contains(@class,'ui-slider-handle')])[5]");
    if ((await sliderHandle.count()) > 0) {
      await sliderHandle.hover();
      const leftslid = await sliderHandle.boundingBox();
      if (leftslid) {
        await page.mouse.down();
        await page.mouse.move(leftslid.x + 120, leftslid.y + leftslid.height / 2);
        await page.mouse.up();
      }
    }
  });
